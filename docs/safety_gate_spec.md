...
The project is a paper-only research scaffold. Safety gates are designed to block live-readiness claims until the paper evidence is strong enough for a separate human security and market-risk review.
...
Any change that adds execution, signing, private-key handling, Web3 order-submission dependencies, or production Polymarket order paths must be treated as out of scope for this repository state.
